...
# def test_load_state_falls_back_to_base_state():
#     """Test load_state falls back to BaseState when no workflow class is registered."""
#     resolver = VellumResolver()
#     execution_id = uuid4()
#     root_execution_id = uuid4()
...
#     # Create mock workflow execution events
#     mock_body_2 = WorkflowExecutionInitiatedBody(
#         workflow_definition=Mock(),
#         inputs={}
#     )

#     previous_invocation_2 = WorkflowExecutionInitiatedEvent(
#         id=str(uuid4()),
#         timestamp=datetime.now(),
#         trace_id=str(uuid4()),
#         span_id=str(execution_id),
...
#     root_invocation_2 = WorkflowExecutionInitiatedEvent(
#         id=str(uuid4()),
#         timestamp=datetime.now(),
#         trace_id=str(uuid4()),
#         span_id=str(root_execution_id),
...
#     # Create mock spans containing the events
#     mock_span_2 = WorkflowExecutionSpan(
#         id=str(uuid4()),
#         start=datetime.now(),
#         events=[previous_invocation_2, root_invocation_2]
#     )

#     mock_response_2 = WorkflowExecutionDetail(
#         span_id="test-span-id", start=datetime.now(), inputs=[], outputs=[], spans=[mock_span_2], state=state_dict
#     )

#     mock_client_2 = Mock()
#     mock_client_2.workflow_executions.retrieve_workflow_execution_detail.return_value = mock_response_2

#     # AND context without a workflow class - create a minimal workflow instance to register
#     context = WorkflowContext(vellum_client=mock_client_2)
#     class MinimalWorkflow(BaseWorkflow[BaseInputs, BaseState]):
#         pass

#     workflow_instance = MinimalWorkflow(context=context, resolvers=[resolver])
#     resolver.register_workflow_instance(workflow_instance)

#     result = resolver.load_state(previous_execution_id=execution_id)

#     # THEN should return LoadStateResult with BaseState when no workflow class is available
#     assert isinstance(result, LoadStateResult)
#     assert result.state is not None
#     assert isinstance(result.state, BaseState)
#     assert result.state["test_key"] == "test_value"
...
#     mock_client_2.workflow_executions.retrieve_workflow_execution_detail.assert_called_once_with(
#         execution_id=str(execution_id)
#     )
