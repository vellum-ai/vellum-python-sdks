...
from vellum.workflows.events.context import get_monitoring_execution_context
...
class EnabledTrivialWorkflow(TrivialWorkflow):
...
def test_basic_workflow_monitoring_context_flow():
    ...
    with patch("vellum.workflows.context.get_execution_context", side_effect=get_monitoring_execution_context):
        ...
        workflow = EnabledTrivialWorkflow()
    ...
    for event in node_events:
        ...
        assert event.parent.workflow_definition.name == "EnabledTrivialWorkflow", "Parent workflow name mismatch"
