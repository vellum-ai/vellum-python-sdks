...
@contextmanager
def mock_composio_environment(api_key="test-api-key", service_return_value=None):
    ...
    with patch("os.environ.get") as mock_env_get:
        mock_env_get.side_effect = api_key if api_key == "COMPOSIO_API_KEY" else None
...
def test_composio_node_extracts_and_executes_with_arguments(composio_node):
    ...
    with mock_composio_environment() as mock_service:
...
def test_composio_node_handles_missing_arguments(composio_node, function_call_output, scenario_name):
    """Test that ComposioNode handles cases where function call arguments are missing or empty."""
    # GIVEN different function call output scenarios
    composio_node.function_call_output = function_call_output

    # WHEN we run the node
    with mock_composio_environment() as mock_service:
        list(composio_node.run())

    # THEN the tool should be executed with empty arguments
    mock_service.return_value.execute_tool.assert_called_once_with(tool_name="GITHUB_CREATE_AN_ISSUE", arguments={})


def test_composio_node_handles_service_errors(composio_node):
    """Test that ComposioNode handles ComposioService execution errors."""
    # WHEN the service raises an exception
    with patch("os.environ.get") as mock_env_get:
        mock_env_get.side_effect = lambda key: "test-api-key" if key == "COMPOSIO_API_KEY" else None
        with patch("vellum.workflows.nodes.displayable.tool_calling_node.utils.ComposioService") as mock_service:
...
def test_composio_node_updates_chat_history(composio_node):
    ...
    # WHEN we run the node
    with mock_composio_environment(service_return_value=expected_result):
        list(composio_node.run())
