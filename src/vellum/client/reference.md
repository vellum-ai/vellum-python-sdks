...
```python
from vellum import CodeExecutionPackage, CodeExecutorInput_String, Vellum
...
```python
from vellum import PromptDeploymentInputRequest_String, Vellum
...
```python
from vellum import Vellum, WorkflowRequestInputRequest_String
...
```python
from vellum import SubmitWorkflowExecutionActualRequest_String, Vellum
...
```python
from vellum import ExecuteToolRequest_Composio, Vellum
...
client.integrations.execute_integration_tool(
    ...
    integration_name="integration_name",
    tool_name="tool_name",
    request=ExecuteToolRequest_Composio(
...
**integration_provider:** `typing.Optional[IntegrationsListRequestIntegrationProvider]` — * `COMPOSIO` - Composio
...
```python
from vellum import MetricDefinitionInput_String, Vellum
...
```python
from vellum import NamedScenarioInputRequest_String, Vellum
...
```python
from vellum import NamedTestCaseVariableValueRequest_String, Vellum
...
**release_tag:** `typing.Optional[str]` — Release tag to use when pulling from deployment (implies deployment-only lookup)
...
**version:** `typing.Optional[str]` — Semantic version range to validate against the Workflow SDK version (e.g., '>=1.0.0,<1.2.3')
