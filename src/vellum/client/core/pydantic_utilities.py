...
def convert_pydantic_to_json_schema(schema_input: typing.Any) -> typing.Dict[str, typing.Any]:
    """
    ...
    if inspect.isclass(schema_input) and issubclass(schema_input, pydantic.BaseModel):
        return _compile_pydantic_model_to_schema(schema_input)
    elif isinstance(schema_input, pydantic.BaseModel):
        return _compile_pydantic_model_to_schema(schema_input.__class__)
    else:
        raise ValueError(f"Expected Pydantic model class/instance or dict, got {type(schema_input)}")


def _compile_pydantic_model_to_schema(model_class: typing.Type[pydantic.BaseModel]) -> typing.Dict[str, typing.Any]:
    """
    Convert a Pydantic model class to JSON schema format.
    """
    properties = {}
    required = []
    ...
    for field_name, field in model_class.model_fields.items():
        ...
        properties[field_name] = _compile_annotation_to_json_type(field_annotation)
...
def _compile_annotation_to_json_type(annotation: typing.Any) -> typing.Dict[str, typing.Any]:
    """
    ...
    if annotation == str:
        ...
        return {"type": "boolean"}
    elif annotation == list or typing_extensions.get_origin(annotation) == list:
        return {"type": "array"}
    elif annotation == dict or typing_extensions.get_origin(annotation) == dict:
        return {"type": "object"}
    else:
        return {"type": "string"}
