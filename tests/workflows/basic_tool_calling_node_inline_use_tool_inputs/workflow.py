...
class StringToolCallingNode(ToolCallingNode):
    """
    A tool calling node that receives inputs from parent workflow using inline use_tool_inputs syntax.
    """
    ...
    # Use inline use_tool_inputs syntax instead of decorator on the function
    functions = [
        use_tool_inputs(
            parent_input=Inputs.parent_input,
...
class BasicToolCallingNodeInlineUseToolInputsWorkflow(BaseWorkflow[Inputs, BaseState]):
    """
    A workflow that uses inline use_tool_inputs syntax on the tool calling node.
    """
