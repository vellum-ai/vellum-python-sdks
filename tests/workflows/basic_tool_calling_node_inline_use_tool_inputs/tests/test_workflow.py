...
from tests.workflows.basic_tool_calling_node_inline_use_tool_inputs.inputs import Inputs
from tests.workflows.basic_tool_calling_node_inline_use_tool_inputs.workflow import (
    BasicToolCallingNodeInlineUseToolInputsWorkflow,
)


def test_tool_calling_node_with_inline_use_tool_inputs(vellum_adhoc_prompt_client, mock_uuid4_generator):
    """
    Tests that inline use_tool_inputs(...)(func) syntax works correctly in a ToolCallingNode.
    """
    ...
    # GIVEN a workflow with tool calling node using inline use_tool_inputs syntax
    workflow = BasicToolCallingNodeInlineUseToolInputsWorkflow()

    # WHEN the workflow is executed with parent inputs
    terminal_event = workflow.run(inputs=Inputs(parent_input="Hello from parent"))
    ...
    assert terminal_event.outputs.chat_history == [
        ...
        ChatMessage(
            ...
            content=StringChatMessageContent(
                type="STRING",
                value='"parent input: Hello from parent, dummy input: dummy, constant input: constant_input, populated input: Hello from user"',  # noqa: E501
            ),
    ...
    # AND the function definition excludes the inputs provided via use_tool_inputs
