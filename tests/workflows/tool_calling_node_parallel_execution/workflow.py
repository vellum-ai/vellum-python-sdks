...
def slow_tool_three() -> str:
    """A tool that takes 0.5 seconds to execute."""
    time.sleep(0.5)
    return "slow_tool_three_result"
...
class ToolCallingNodeParallelExecution(ToolCallingNode):
    ...
    Currently executes sequentially (~1.5s total), should execute in parallel (~0.5s total).
    """
    ...
    functions = [slow_tool_one, slow_tool_two, slow_tool_three]
    prompt_inputs = {
