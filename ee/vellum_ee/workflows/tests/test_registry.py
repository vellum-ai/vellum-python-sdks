...
def test_get_parent_display_context_from_event__workflow_parent_found():
    """Test event with workflow parent returns display context if registered"""
    # GIVEN a workflow parent context
    workflow_span_id = uuid4()
    workflow_parent = WorkflowParentContext(workflow_definition=MockWorkflow, span_id=workflow_span_id, parent=None)
    ...
    # AND the workflow display context is registered
    context = MockWorkflowDisplayContext("workflow_context")
    register_workflow_display_context(workflow_span_id, context)

    # WHEN getting parent display context
    result = get_parent_display_context_from_event(event)

    # THEN it should return the registered context
...
def test_get_parent_display_context_from_event__middle_workflow_has_context():
    ...
    # Event -> NodeParent -> MiddleWorkflowParent (with context) -> TopWorkflowParent (with context)

    # Top level workflow parent (has display context)
    top_workflow_span_id = uuid4()
    top_context = MockWorkflowDisplayContext("top_context")
    register_workflow_display_context(str(top_workflow_span_id), top_context)

    top_workflow_parent = WorkflowParentContext(
        workflow_definition=MockWorkflow, span_id=top_workflow_span_id, parent=None
    )

    # AND middle workflow parent (HAS display context - this is the key difference!)
    middle_workflow_span_id = uuid4()
    middle_context = MockWorkflowDisplayContext("middle_context")
    register_workflow_display_context(str(middle_workflow_span_id), middle_context)
    ...
    # THEN it should find the MIDDLE workflow context (first one with registered context)
