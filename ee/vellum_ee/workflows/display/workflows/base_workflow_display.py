...
class BaseWorkflowDisplay(Generic[WorkflowType]):
    ...
    def serialize(self) -> JsonObject:
        ...
        # For ManualTrigger: create ENTRYPOINT node for backward compatibility
        ...
        elif not trigger_edges:
            # No trigger: use default ENTRYPOINT node (backward compatibility)
        ...
        if has_only_integration_trigger:
            ...
            # Traditional approach: edges from ENTRYPOINT node (backward compatibility)
            for target_node, entrypoint_display in self.display_context.entrypoint_displays.items():
                unadorned_target_node = get_unadorned_node(target_node)
                # Skip edges to invalid nodes
                if self._is_node_invalid(unadorned_target_node):
        ...
        nodes_with_explicit_positions = 0
        total_nodes_with_position_data = 0

        for node_dict in nodes_dict_list:
            if isinstance(node_dict.get("display_data"), dict) and isinstance(
                node_dict["display_data"].get("position"), dict
            ):
                total_nodes_with_position_data += 1
                position = node_dict["display_data"]["position"]
                x = position.get("x", 0)
                y = position.get("y", 0)
                if x != 0.0 or y != 0.0:
                    nodes_with_explicit_positions += 1

        should_apply_auto_layout = (
            len(nodes_dict_list) > 0 and total_nodes_with_position_data > 0 and nodes_with_explicit_positions == 0
        )
