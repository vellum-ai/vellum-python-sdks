...
def auto_layout_selected_nodes(
    ...
    """
    Auto-layout only the selected nodes while preserving positions of others.

    Args:
        nodes: List of all (node_id, NodeDisplayData) tuples
        edges: List of (source_node_id, target_node_id, EdgeDisplay) tuples
        selected_node_ids: Set of node IDs to layout
        node_spacing: Vertical spacing between nodes in the same layer
        layer_spacing: Horizontal spacing between layers

    Returns:
        List of (node_id, NodeDisplayData) tuples with updated positions for selected nodes
    """
    if not selected_node_ids:
        return nodes

    selected_nodes = [(node_id, data) for node_id, data in nodes if node_id in selected_node_ids]
    selected_edges = [
    ...
    positioned_selected = auto_layout_nodes(selected_nodes, selected_edges, node_spacing, layer_spacing)

    result = []
    positioned_dict = {node_id: data for node_id, data in positioned_selected}

    for node_id, original_data in nodes:
        if node_id in positioned_dict:
            result.append((node_id, positioned_dict[node_id]))
        else:
            result.append((node_id, original_data))
