...
const main = async () => {
  ...
  execSync(`npm install @vellum-ai/vellum-codegen-${version}@npm:@vellum-ai/vellum-codegen@${version}  --save-exact`, {
    stdio: "inherit",
  });
  ...
  const codegenPackageJson = JSON.parse(readFileSync(path.join(targetDir, "package.json"), 'utf8'));
...
`
  writeFileSync(path.join(targetDir, "src/utils/getCodegenModule.ts"), codeGeneratorFunction);
