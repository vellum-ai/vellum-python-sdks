...
describe("WorkflowProjectGenerator", () => {
  ...
  describe("generateCodeNodeDirectoryOnly", () => {
    ...
    it("should generate metadata.json with dataset row index to id mapping", async () => {
      ...
      const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf-8"));
      expect(metadata.dataset_row_index_to_id_mapping).toEqual({
        "0": "dataset-row-id-1",
        "1": "dataset-row-id-2",
      });
    });
  ...
  describe("include sandbox", () => {
    ...
    it("should generate correct inputs for non-empty list and dict defaults", async () => {
      const displayDataWithDefaults = {
        workflow_raw_data: {
          nodes: [
            {
        ...
        input_variables: [
          {
            key: "non_empty_list_input",
            type: "ARRAY",
            id: "non-empty-list-input",
            required: true,
            default: {
              type: "ARRAY",
              value: [
          ...
          {
            key: "non_empty_dict_input",
            type: "JSON",
            id: "non-empty-dict-input",
            required: true,
            default: {
              type: "JSON",
          ...
          {
            key: "non_empty_list_input_optional",
            type: "ARRAY",
            id: "non-empty-list-input-optional",
            required: false,
            default: {
              type: "ARRAY",
          ...
          {
            key: "non_empty_dict_input_optional",
            type: "JSON",
            id: "non-empty-dict-input-optional",
            required: false,
            default: {
              type: "JSON",
...
`,
  ...
  describe("sibling directory preservation", () => {
    it("should preserve sibling directories next to nodes during codegen", async () => {
      /**
       * Tests that codegen does not delete sibling directories next to nodes.
       */

      // GIVEN a pre-existing sibling directory next to nodes
      const siblingDir = join(tempDir, "code", "integration_classes");
      await mkdir(siblingDir, { recursive: true });
      const sentinelFilePath = join(siblingDir, "sentinel.py");
      const sentinelContent = "# This file should persist through codegen\n";
      fs.writeFileSync(sentinelFilePath, sentinelContent);

      // AND a simple workflow configuration
      const displayData = {
        workflow_raw_data: {
          nodes: [
            {
            ...
            },
            {
              ...
              data: {
                label: "Templating Node",
                template_node_input_id: "template-input",
                output_id: "output-id",
                output_type: "STRING",
                source_handle_id: "template-source",
                target_handle_id: "template-target",
              },
              inputs: [
                {
                  id: "template-input",
      ...
      // THEN the sibling directory should still exist
      expect(
        fs.existsSync(siblingDir),
        "Sibling directory 'integration_classes' should persist after codegen"
      ).toBe(true);

      // AND the sentinel file should still exist with unchanged content
      expect(
        fs.existsSync(sentinelFilePath),
        "Sentinel file should persist after codegen"
      ).toBe(true);
      expect(fs.readFileSync(sentinelFilePath, "utf-8")).toBe(sentinelContent);

      // AND the generated nodes directory should exist
      expectProjectFileToExist(["code", "nodes", "templating_node.py"]);
    });

    it("should preserve multiple sibling directories next to nodes during codegen", async () => {
      /**
       * Tests that codegen preserves multiple sibling directories.
       */

      // GIVEN multiple pre-existing sibling directories next to nodes
      const siblingDir1 = join(tempDir, "code", "integration_classes");
      const siblingDir2 = join(tempDir, "code", "custom_utils");
      await mkdir(siblingDir1, { recursive: true });
      await mkdir(siblingDir2, { recursive: true });

      const sentinelFile1 = join(siblingDir1, "composio_models.py");
      const sentinelFile2 = join(siblingDir2, "helpers.py");
      const content1 =
        "# Composio integration models\nclass MyModel:\n    pass\n";
      const content2 = "# Custom utility functions\ndef helper():\n    pass\n";
      fs.writeFileSync(sentinelFile1, content1);
      fs.writeFileSync(sentinelFile2, content2);
      ...
      // THEN both sibling directories should still exist
      expect(
        fs.existsSync(siblingDir1),
        "Sibling directory 'integration_classes' should persist after codegen"
      ).toBe(true);
      expect(
        fs.existsSync(siblingDir2),
        "Sibling directory 'custom_utils' should persist after codegen"
      ).toBe(true);

      // AND the sentinel files should still exist with unchanged content
      expect(fs.readFileSync(sentinelFile1, "utf-8")).toBe(content1);
      expect(fs.readFileSync(sentinelFile2, "utf-8")).toBe(content2);
    });
  });
});
